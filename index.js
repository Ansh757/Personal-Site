...
*/
    ...
    function drawLine(box) {
        ...
        let item = document.querySelector('.__item');
        let text = document.querySelector('.__item .text');

        let distanceBetweenItemAndLine = (item.offsetWidth / 4) - text.offsetWidth;

        if (box.classList.contains('__left')) {
        connector.style.left = text.offsetWidth  + 380 + 'px';
        } else {
        connector.style.right = text.offsetWidth + 380 + 'px';
        }
